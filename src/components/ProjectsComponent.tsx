...
export const ProjectsComponent = (props: ProjectsProps) => {
  ...
  function projectRender() {
    ...
    } else if (props.competence) {
      return projects
        .filter((p) => p.competence)
        .map((p) => <ProjectCard key={p.title} project={p} />);
    } else {
      return projects
        .filter((p) => !p.misc && !p.competence)
        .map((p) => <ProjectCard key={p.title} project={p} />);
    }
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-4">{projectRender()}</div>
    </div>
  );
};
